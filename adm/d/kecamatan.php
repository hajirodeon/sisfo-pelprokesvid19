...
if ($_POST['btnSMP'])
	...
	if (empty($e_nama))
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE m_kecamatan SET kecamatan = '$e_nama' ".
							"WHERE kd = '$kd'");
		...
		if ($s == "baru")
			{
			//cek
			$qcc = mysqli_query($koneksi, "SELECT * FROM m_kecamatan ".
												"WHERE kecamatan = '$e_nama'");
			...
			if ($tcc != 0)
				...
				{
				mysqli_query($koneksi, "INSERT INTO m_kecamatan(kd, kecamatan, postdate) VALUES ".
								"('$kd', '$e_nama', '$today')");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM m_kecamatan ".
						"WHERE kd = '$kd'");
		}
...
if (($s == "baru") OR ($s == "edit"))
	...
	$qx = mysqli_query($koneksi, "SELECT * FROM m_kecamatan ".
						"WHERE kd = '$kdx'");
	$rowx = mysqli_fetch_assoc($qx);
	$e_nama = balikin($rowx['kecamatan']);
	?>
	...
	$(document).ready(function () {
		...
		$('#e_nama').bind('keyup paste', function(){
	...
	if (empty($kunci))
		{
		$sqlcount = "SELECT * FROM m_kecamatan ".
	...
	<td width="20">&nbsp;</td>
	<td width="20">&nbsp;</td>
	<td><strong><font color="'.$warnatext.'">NAMA KECAMATAN</font></strong></td>
	</tr>
	</thead>
	<tbody>';
